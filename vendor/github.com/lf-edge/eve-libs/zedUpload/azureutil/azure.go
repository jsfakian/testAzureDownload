...
func getContainerClient(accountURL, accountName, accountKey, containerName string) (*container.Client, error) {
	cred, err := azblob.NewSharedKeyCredential(accountName, accountKey)
	if err != nil {
		return nil, fmt.Errorf("failed to create credential: %v", err)
	}

	serviceURL := strings.TrimSuffix(accountURL, "/")
...
// getContainerAndBlockBlobClients returns both a container client and a block blob client.
func getContainerAndBlockBlobClients(
	...
	containerClient, err := getContainerClient(accountURL, accountName, accountKey, containerName)
	if err != nil {
		return nil, nil, err
	}

	// Only this line is specific to this helper
	blobClient := containerClient.NewBlockBlobClient(remoteFile)
...
func ListAzureBlob(accountURL, accountName, accountKey, containerName string) ([]string, error) {
	var imgList []string
...
func DeleteAzureBlob(accountURL, accountName, accountKey, containerName, remoteFile string) error {
	_, blobClient, err := getContainerAndBlockBlobClients(accountURL, accountName, accountKey, containerName, remoteFile)
	if err != nil {
...
// DownloadAzureBlob: Parallel, resumable, chunked download with progress.
// Steps:
...
func DownloadAzureBlob(
	...
	objMaxSize int64, doneParts types.DownloadedParts, prgNotify types.StatsNotifChan,
) (types.DownloadedParts, error) {

	stats := &types.UpdateStats{DoneParts: doneParts}

	_, blobClient, err := getContainerAndBlockBlobClients(accountURL, accountName, accountKey, containerName, blobName)
	if err != nil {
		fmt.Errorf("Error: %v", err)
	}
...
func UploadAzureBlob(
	...
	// Get clients using helper
	containerClient, blobClient, err := getContainerAndBlockBlobClients(accountURL, accountName, accountKey, containerName, remoteFile)
	if err != nil {
...
func GenerateBlobSasURI(
	...
	_, _, err = GetAzureBlobMetaData(accountURL, accountName, accountKey, containerName, remoteFile)
	if err != nil {
